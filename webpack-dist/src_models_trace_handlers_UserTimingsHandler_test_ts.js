"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_models_trace_handlers_UserTimingsHandler_test_ts"],{

/***/ "./src/models/trace/handlers/UserTimingsHandler.test.ts":
/*!**************************************************************!*\
  !*** ./src/models/trace/handlers/UserTimingsHandler.test.ts ***!
  \**************************************************************/
...
describe('UserTimingsHandler', function () {
    ...
    describe('performance timings', function () {
        async function getTimingsDataFromEvents(events) {
            ...
            await _trace_js__WEBPACK_IMPORTED_MODULE_1__.Handlers.ModelHandlers.UserTimings.finalize();
            return _trace_js__WEBPACK_IMPORTED_MODULE_1__.Handlers.ModelHandlers.UserTimings.data();
        }
        before(async function () {
            const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_0__.TraceLoader.rawEvents(this, 'user-timings.json.gz');
        ...
        describe('performance.measure events parsing', function () {
            it('parses the start and end events and returns a list of blocks', async () => {
                ...
                for (let i = 0; i < timingsData.performanceMeasures.length; i++) {
                    assert.strictEqual(timingsData.performanceMeasures[i].args.data.beginEvent.ph, _trace_js__WEBPACK_IMPORTED_MODULE_1__.Types.TraceEvents.Phase.ASYNC_NESTABLE_START);
                    assert.strictEqual(timingsData.performanceMeasures[i].args.data.endEvent.ph, _trace_js__WEBPACK_IMPORTED_MODULE_1__.Types.TraceEvents.Phase.ASYNC_NESTABLE_END);
                }
            });
            it('sorts the blocks to ensure they are in time order', async function () {
                ...
                await _trace_js__WEBPACK_IMPORTED_MODULE_1__.Handlers.ModelHandlers.UserTimings.finalize();
                const data = _trace_js__WEBPACK_IMPORTED_MODULE_1__.Handlers.ModelHandlers.UserTimings.data();
            ...
            it('correctly extracts nested timings in the correct order', async function () {
                const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_0__.TraceLoader.rawEvents(this, 'user-timings-complex.json.gz');
    ...
    describe('console timings', function () {
        before(async function () {
            const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_0__.TraceLoader.rawEvents(this, 'timings-track.json.gz');
            _trace_js__WEBPACK_IMPORTED_MODULE_1__.Handlers.ModelHandlers.UserTimings.reset();
            for (const event of events) {
                _trace_js__WEBPACK_IMPORTED_MODULE_1__.Handlers.ModelHandlers.UserTimings.handleEvent(event);
            }
        ...
        describe('console.time events parsing', function () {
            it('parses the start and end events and returns a list of blocks', async () => {
                ...
                for (let i = 0; i < timingsData.consoleTimings.length; i++) {
                    assert.strictEqual(timingsData.consoleTimings[i].args.data.beginEvent.ph, _trace_js__WEBPACK_IMPORTED_MODULE_1__.Types.TraceEvents.Phase.ASYNC_NESTABLE_START);
                    assert.strictEqual(timingsData.consoleTimings[i].args.data.endEvent.ph, _trace_js__WEBPACK_IMPORTED_MODULE_1__.Types.TraceEvents.Phase.ASYNC_NESTABLE_END);
                }
            });
            it('sorts the blocks to ensure they are in time order', async function () {
                const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_0__.TraceLoader.rawEvents(this, 'timings-track.json.gz');
