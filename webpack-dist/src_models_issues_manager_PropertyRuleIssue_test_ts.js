"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_models_issues_manager_PropertyRuleIssue_test_ts"],{

/***/ "./src/models/issues_manager/PropertyRuleIssue.test.ts":
/*!*************************************************************!*\
  !*** ./src/models/issues_manager/PropertyRuleIssue.test.ts ***!
  \*************************************************************/
...
function createProtocolIssue(propertyRuleIssueDetails) {
    return {
        code: _generated_protocol_js__WEBPACK_IMPORTED_MODULE_0__.Audits.InspectorIssueCode.PropertyRuleIssue,
        details: { propertyRuleIssueDetails },
    };
}
(0,_testing_EnvironmentHelpers_js__WEBPACK_IMPORTED_MODULE_2__.describeWithLocale)('PropertyRuleIssue', () => {
    ...
    it('can be created for property rules parsing failures', () => {
        const issueDetails = {
            ...
            propertyRuleIssueReason: _generated_protocol_js__WEBPACK_IMPORTED_MODULE_0__.Audits.PropertyRuleIssueReason.InvalidInitialValue,
            propertyValue: 'invalid',
        };
        const issue = createProtocolIssue(issueDetails);
        const propertyRuleIssues = _issues_manager_issues_manager_js__WEBPACK_IMPORTED_MODULE_5__.PropertyRuleIssue.PropertyRuleIssue.fromInspectorIssue(mockModel, issue);
        assert.lengthOf(propertyRuleIssues, 1);
        const propertyRuleIssue = propertyRuleIssues[0];
        assert.strictEqual(propertyRuleIssue.getCategory(), _issues_manager_issues_manager_js__WEBPACK_IMPORTED_MODULE_5__.Issue.IssueCategory.Other);
        assert.deepStrictEqual(propertyRuleIssue.sources(), [issueDetails.sourceCodeLocation]);
        assert.strictEqual(propertyRuleIssue.getKind(), _issues_manager_issues_manager_js__WEBPACK_IMPORTED_MODULE_5__.Issue.IssueKind.PageError);
    ...
    it('only aggregates identical issues', () => {
        const issueDetails = [
            ...
            },
            {
                ...
                propertyRuleIssueReason: _generated_protocol_js__WEBPACK_IMPORTED_MODULE_0__.Audits.PropertyRuleIssueReason.InvalidInherits,
                propertyValue: 'invalid',
            },
            {
                sourceCodeLocation: {
        ...
        const issues = issueDetails
            .map(details => _issues_manager_issues_manager_js__WEBPACK_IMPORTED_MODULE_5__.PropertyRuleIssue.PropertyRuleIssue.fromInspectorIssue(mockModel, createProtocolIssue(details)))
            .flat();
