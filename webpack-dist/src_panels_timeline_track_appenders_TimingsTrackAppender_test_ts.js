"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_panels_timeline_track_appenders_TimingsTrackAppender_test_ts"],{

/***/ "./src/panels/timeline/track_appenders/TimingsTrackAppender.test.ts":
/*!**************************************************************************!*\
  !*** ./src/panels/timeline/track_appenders/TimingsTrackAppender.test.ts ***!
  \**************************************************************************/
...
(0,_testing_EnvironmentHelpers_js__WEBPACK_IMPORTED_MODULE_1__.describeWithEnvironment)('TimingTrackAppender', function () {
    ...
    beforeEach(async function () {
        ({ traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_2__.TraceLoader.traceEngine(this, 'timings-track.json.gz'));
    ...
    describe('appendTrackAtLevel', () => {
        it('marks all levels used by the track with the `TrackAppender` type', () => {
            ...
            const allEntriesAreTrackAppender = entryTypeByLevel.every(type => type === _timeline_js__WEBPACK_IMPORTED_MODULE_5__.TimelineFlameChartDataProvider.EntryType.TrackAppender);
        ...
        it('creates a TimelineFlameChartMarker for each page load marker event in a trace', () => {
            ...
            for (const traceMarker of traceMarkers) {
                const markerTimeMs = _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Helpers.Timing.microSecondsToMilliseconds(traceMarker.ts);
    ...
    describe('colorForEvent and titleForEvent', () => {
        it('returns the correct color and title for page load markers', () => {
            ...
            assert.strictEqual(timingsTrackAppender.titleForEvent(firstContentfulPaint), _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.FCP);
            assert.strictEqual(timingsTrackAppender.colorForEvent(markLoad), '#B31412');
            assert.strictEqual(timingsTrackAppender.titleForEvent(markLoad), _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.L);
            assert.strictEqual(timingsTrackAppender.colorForEvent(markDOMContent), '#0867CB');
            assert.strictEqual(timingsTrackAppender.titleForEvent(markDOMContent), _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.DCL);
            assert.strictEqual(timingsTrackAppender.colorForEvent(firstPaint), '#228847');
            assert.strictEqual(timingsTrackAppender.titleForEvent(firstPaint), _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.FP);
            assert.strictEqual(timingsTrackAppender.colorForEvent(largestContentfulPaint), '#1A3422');
            assert.strictEqual(timingsTrackAppender.titleForEvent(largestContentfulPaint), _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.LCP);
        });
