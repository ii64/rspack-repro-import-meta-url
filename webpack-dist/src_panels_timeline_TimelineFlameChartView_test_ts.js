"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_panels_timeline_TimelineFlameChartView_test_ts"],{

/***/ "./src/panels/timeline/TimelineFlameChartView.test.ts":
/*!************************************************************!*\
  !*** ./src/panels/timeline/TimelineFlameChartView.test.ts ***!
  \************************************************************/
...
(0,_testing_EnvironmentHelpers_js__WEBPACK_IMPORTED_MODULE_2__.describeWithEnvironment)('TimelineFlameChartView', function () {
    ...
    describe('groupForLevel', () => {
        const { groupForLevel } = _timeline_js__WEBPACK_IMPORTED_MODULE_7__.TimelineFlameChartView;
    ...
    it('Can search for events by name in the timeline', async function () {
        const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_4__.TraceLoader.traceEngine(this, 'lcp-images.json.gz');
        ...
        const flameChartView = new _timeline_js__WEBPACK_IMPORTED_MODULE_7__.TimelineFlameChartView.TimelineFlameChartView(mockViewDelegate);
        const searchableView = new _ui_legacy_legacy_js__WEBPACK_IMPORTED_MODULE_6__.SearchableView.SearchableView(flameChartView, null);
        flameChartView.setSearchableView(searchableView);
        flameChartView.setModel(traceData);
        const searchQuery = 'Paint';
        const searchConfig = new _ui_legacy_legacy_js__WEBPACK_IMPORTED_MODULE_6__.SearchableView.SearchConfig(/* query */ searchQuery, /* caseSensitive */ false, /* isRegex */ false);
    ...
    it.skip('[crbug.com/1492405] Shows the network track correctly', async function () {
        const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_4__.TraceLoader.traceEngine(this, 'load-simple.json.gz');
        // The timeline flamechart view will invoke the `select` method
        // of this delegate every time an event has matched on a search.
        const mockViewDelegate = new MockViewDelegate();
        const flameChartView = new _timeline_js__WEBPACK_IMPORTED_MODULE_7__.TimelineFlameChartView.TimelineFlameChartView(mockViewDelegate);
    ...
    it('Does not show the network track when there is no network request', async function () {
        const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_4__.TraceLoader.traceEngine(this, 'basic.json.gz');
        // The timeline flamechart view will invoke the `select` method
        // of this delegate every time an event has matched on a search.
        const mockViewDelegate = new MockViewDelegate();
        const flameChartView = new _timeline_js__WEBPACK_IMPORTED_MODULE_7__.TimelineFlameChartView.TimelineFlameChartView(mockViewDelegate);
    ...
    it('Adds Hidden Descendants Arrow as a decoration when a Context Menu action is applied on a node', async function () {
        ...
        flameChartView.getMainFlameChart().modifyTree(_ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_5__.FlameChart.FilterAction.COLLAPSE_FUNCTION, node?.id);
        ...
        assert.deepEqual(decorationsForEntry, [
            {
                type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_5__.FlameChart.FlameChartDecorationType.HIDDEN_DESCENDANTS_ARROW,
            },
        ]);
    });
    ...
    it('Removes Hidden Descendants Arrow as a decoration when Reset Children action is applied on a node', async function () {
        ...
        flameChartView.getMainFlameChart().modifyTree(_ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_5__.FlameChart.FilterAction.RESET_CHILDREN, node?.id);
    ...
    describe('Context Menu', function () {
        ...
        this.beforeEach(async () => {
            ({ traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_4__.TraceLoader.traceEngine(this, 'recursive-blocking-js.json.gz'));
            const mockViewDelegate = new MockViewDelegate();
        ...
        describe('Context Menu Actions For Thread tracks', function () {
            ...
            it('Reset Trace Context Menu action is disabled before some action has been applied', async function () {
                ...
                assert.strictEqual(flameChartView.getMainFlameChart()
                    ...
                    ?.buildDescriptor()
                    .enabled, false);
                flameChartView.getMainFlameChart().modifyTree(_ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_5__.FlameChart.FilterAction.MERGE_FUNCTION, nodeId);
                generateContextMenuForNodeId(nodeId);
