"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_core_platform_StringUtilities_test_ts"],{

/***/ "./src/core/platform/StringUtilities.test.ts":
/*!***************************************************!*\
  !*** ./src/core/platform/StringUtilities.test.ts ***!
  \***************************************************/
...
describe('StringUtilities', () => {
    describe('escapeCharacters', () => {
        it('escapes the given characters', () => {
            ...
            const charsToEscape = '\'';
            const outputString = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.escapeCharacters(inputString, charsToEscape);
    ...
    describe('toBase64', () => {
        it('encodes correctly and supports unicode characters', () => {
            ...
            for (const [inputString, encodedString] of fixtures) {
                assert.strictEqual(encodedString, _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.toBase64(inputString), `failed to encode ${inputString} correctly`);
            }
        });
    });
    describe('findIndexesOfSubstring', () => {
        it('finds the expected indexes', () => {
            ...
            const indexes = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.findIndexesOfSubString(inputString, 'F');
...
78
9`;
            const indexes = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.findLineEndingIndexes(inputString);
    ...
    describe('collapseWhitespace', () => {
        it('collapses consecutive whitespace chars down to a single one', () => {
            ...
            const outputString = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.collapseWhitespace(inputString);
    ...
    describe('replaceControlCharacters', () => {
        it('replaces C0 and C1 control character sets with the replacement character', () => {
            ...
            const outputString = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.replaceControlCharacters(inputString);
    ...
    describe('countWtf8Bytes', () => {
        it('produces the correct WTF-8 byte size', () => {
            ...
            assert.strictEqual(_platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.countWtf8Bytes(`${leadSurrogate}${trailSurrogate}`), 4);
            assert.strictEqual(_platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.countWtf8Bytes(`${trailSurrogate}${leadSurrogate}`), 6);
            assert.strictEqual(_platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.countWtf8Bytes(`${leadSurrogate}`), 3);
            assert.strictEqual(_platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.countWtf8Bytes(`${trailSurrogate}`), 3);
        });
    });
    ...
    describe('isExtendedKebab', () => {
        const { isExtendedKebabCase } = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities;
    ...
    describe('toTitleCase', () => {
        it('converts a string to title case', () => {
            const output = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.toTitleCase('foo bar baz');
    ...
    describe('filterRegex', () => {
        it('should prepend [^\\0 ]* patterns for all characters', () => {
            const regex = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.filterRegex('bar');
            assert.strictEqual(regex.toString(), '/^(?:.*\\0)?[^\\0b]*b[^\\0a]*a[^\\0r]*r/i');
        });
        it('should escape special characters', () => {
            const regex = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.filterRegex('{?}');
        ...
        it('should match strings that have the query characters in the same order', () => {
            ...
            for (const { query, pos, neg } of testCases) {
                const regex = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.filterRegex(query);
    ...
    describe('createSearchRegex', () => {
        it('returns a case sensitive regex if the call states it is case sensitive', () => {
            const regex = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.createSearchRegex('foo', true, false);
        ...
        it('creates a regex from plain text if the given input is not already a regex', () => {
            const regex = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.createSearchRegex('[foo]', false, false);
            assert.strictEqual(regex.source, '\\[foo\\]');
        });
        it('leaves the input be if it is already a regex', () => {
            const regex = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.createSearchRegex('[foo]', false, true);
            assert.strictEqual(regex.source, '[foo]');
        });
    });
    describe('hashCode', () => {
        it('hashes strings', () => {
            ...
            const hashA = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.hashCode(stringA);
    ...
    describe('compare', () => {
        it('returns 1 if the string is > the other string', () => {
            const result = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.compare('b', 'a');
            assert.strictEqual(result, 1);
        });
        it('returns -1 if the string is < the other string', () => {
            const result = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.compare('a', 'b');
            assert.strictEqual(result, -1);
        });
        it('returns 0 if the strings are equal', () => {
            const result = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.compare('a', 'a');
        ...
        for (let i = 0; i < fixtures.length; i++) {
            ...
            it(`trims the middle of strings, fixture ${i}`, () => {
                for (let maxLength = string.length + 1; maxLength > 0; --maxLength) {
                    const trimmed = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.trimMiddle(string, maxLength);
    ...
    describe('escapeForRegExp', () => {
        it('escapes regex characters', () => {
            const inputString = '^[]{}()\\.^$*+?|-';
            const outputString = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.escapeForRegExp(inputString);
        ...
        it('sorts natural order', () => {
            ...
            for (let i = 0, n = testArray.length; i < n; ++i) {
                for (let j = 0; j < n; ++j) {
                    const a = testArray[i];
                    const b = testArray[j];
                    const diff = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.naturalOrderComparator(a, b);
                    if (diff === 0) {
    ...
    describe('formatAsJSLiteral', () => {
        it('wraps plain string in single quotes', () => {
            const inputString = 'foo';
            assert.strictEqual(String.raw `'foo'`, _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.formatAsJSLiteral(inputString));
        });
        it('wraps string containing single quotes in double quotes', () => {
            const inputString = String.raw `'foo' and 'bar'`;
            assert.strictEqual(String.raw `"'foo' and 'bar'"`, _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.formatAsJSLiteral(inputString));
        });
        ...
        it('should not escape valid surrogate pairs', () => {
            ...
            assert.strictEqual(`'${inputString}'`, _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.formatAsJSLiteral(inputString));
        });
    ...
    describe('sprintf', () => {
        ...
        it('deals with parameter indices', () => {
            ...
            assert.throws(() => _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.sprintf('%0$s', 'World'));
        });
        it('signals error when too few parameters are given', () => {
            assert.throws(() => _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.sprintf('%2$s', 'World'));
            assert.throws(() => _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.sprintf('%2$s %s!', 'World', 'Hello'));
            assert.throws(() => _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.sprintf('%s %d', 'World'));
        });
    });
    describe('LowerCaseString', () => {
        ...
        const lower = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.toLowerCaseString('lower case string');
        fnExpectingLowerCaseString(lower);
    });
    describe('replaceLast', () => {
        it('should return the input string when the search is not found', () => {
            const output = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.replaceLast('input', 'search', 'repl');
            assert.strictEqual(output, 'input');
        });
        it('should replace the occurrance when the search exists inside the input', () => {
            const output = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.replaceLast('input', 'pu', 'r');
            assert.strictEqual(output, 'inrt');
        });
        it('should replace the last occurrence when there are multiple matches', () => {
            const output = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.replaceLast('inpuput', 'pu', 'r');
    ...
    describe('toKebabCase', () => {
        const toKebabCase = _platform_js__WEBPACK_IMPORTED_MODULE_0__.StringUtilities.toKebabCase;
