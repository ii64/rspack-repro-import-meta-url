"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_models_source_map_scopes_NamesResolver_test_ts"],{

/***/ "./src/models/source_map_scopes/NamesResolver.test.ts":
/*!************************************************************!*\
  !*** ./src/models/source_map_scopes/NamesResolver.test.ts ***!
  \************************************************************/
...
/* harmony import */ var _testing_MockScopeChain_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../../testing/MockScopeChain.js */ "./src/testing/MockScopeChain.ts");
/* harmony import */ var _bindings_bindings_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ../bindings/bindings.js */ "./src/models/bindings/bindings.ts");
/* harmony import */ var _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../source_map_scopes/source_map_scopes.js */ "./src/models/source_map_scopes/source_map_scopes.ts");
...
(0,_testing_MockConnection_js__WEBPACK_IMPORTED_MODULE_3__.describeWithMockConnection)('NameResolver', () => {
    ...
    beforeEach(() => {
        ...
        const resourceMapping = new _bindings_bindings_js__WEBPACK_IMPORTED_MODULE_5__.ResourceMapping.ResourceMapping(targetManager, workspace);
        const debuggerWorkspaceBinding = _bindings_bindings_js__WEBPACK_IMPORTED_MODULE_5__.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance({
            forceNew: true,
            resourceMapping,
            targetManager,
        });
        _bindings_bindings_js__WEBPACK_IMPORTED_MODULE_5__.IgnoreListManager.IgnoreListManager.instance({ forceNew: true, debuggerWorkspaceBinding });
    ...
    it('test helper parses identifiers from test descriptor', () => {
        ...
        assert.deepEqual(identifiers.bound, [
            new _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__.NamesResolver.IdentifierPositions('a', [{ lineNumber: 0, columnNumber: 27 }, { lineNumber: 0, columnNumber: 41 }]),
        ]);
        assert.deepEqual(identifiers.free, [
            new _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__.NamesResolver.IdentifierPositions('x', [{ lineNumber: 0, columnNumber: 31 }]),
        ]);
    });
    const tests = [
        {
    ...
    for (const test of tests) {
        it(test.name, async () => {
            ...
            const parsedScopeChain = await _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__.NamesResolver.findScopeChainForDebuggerScope(callFrame.scopeChain()[0]);
            const scope = parsedScopeChain.pop();
            assert.exists(scope);
            const identifiers = await _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__.NamesResolver.scopeIdentifiers(callFrame.script, scope, parsedScopeChain);
    ...
    it('resolves name tokens merged with commas (without source map names)', async () => {
        ...
        const resolvedScopeObject = await _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__.NamesResolver.resolveScopeInObject(callFrame.scopeChain()[0]);
    ...
    describe('Function name resolving', () => {
        ...
        it('resolves function names at scope start for a debugger frame', async () => {
            const functionName = await _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__.NamesResolver.resolveDebuggerFrameFunctionName(callFrame);
        ...
        it('resolves function names at scope start for a profiler frame', async () => {
            ...
            const functionName = await _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__.NamesResolver.resolveProfileFrameFunctionName({ scriptId, columnNumber, lineNumber }, target);
            assert.strictEqual(functionName, 'unminified');
        });
    });
    ...
    it('ignores the argument name during arrow function name resolution', async () => {
        ...
        const callFrame = await backend.createCallFrame(target, { url: URL, content: source }, scopes, { url: sourceMapUrl, content: sourceMapContent }, [scopeObject]);
        assert.isNull(await _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__.NamesResolver.resolveDebuggerFrameFunctionName(callFrame));
    });
...
`;
        ...
        it('has the right mapping on a function scope without shadowing', async () => {
            ...
            const mapping = await _source_map_scopes_source_map_scopes_js__WEBPACK_IMPORTED_MODULE_6__.NamesResolver.allVariablesAtPosition(location);
