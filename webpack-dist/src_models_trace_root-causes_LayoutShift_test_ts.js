"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_models_trace_root-causes_LayoutShift_test_ts"],{

/***/ "./src/models/trace/root-causes/LayoutShift.test.ts":
/*!**********************************************************!*\
  !*** ./src/models/trace/root-causes/LayoutShift.test.ts ***!
  \**********************************************************/
...
/* harmony import */ var _RootCauses_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./RootCauses.js */ "./src/models/trace/root-causes/RootCauses.ts");
...
(0,_testing_MockConnection_js__WEBPACK_IMPORTED_MODULE_1__.describeWithMockConnection)('LayoutShift root causes', () => {
    /*
    ...
    describe('assigns root causes to layout shifts', () => {
        ...
        beforeEach(() => {
            ...
            for (let i = 0; i < resizeEvents.length; i++) {
                resizeEvents[i].args = {
                    data: {
                        nodeId: i + 1,
                        reason: _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.TraceEvents.LayoutInvalidationReason.SIZE_CHANGED,
                        nodeName: 'IMG',
                        frame: 'frame-id-123',
                    },
                };
            ...
            for (let i = 0; i < injectedIframeEvents.length; i++) {
                injectedIframeEvents[i].args = {
                    data: {
                        nodeId: i + 11,
                        reason: _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.TraceEvents.LayoutInvalidationReason.ADDED_TO_LAYOUT,
                        nodeName: 'IFRAME',
                        frame: 'frame-id-123',
                    },
                };
            ...
            for (let i = 0; i < fontChanges.length; i++) {
                fontChanges[i].args = {
                    data: {
                        nodeId: i + 21,
                        reason: _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.TraceEvents.LayoutInvalidationReason.FONTS_CHANGED,
                        nodeName: 'DIV',
                        frame: 'frame-id-123',
                    },
                };
            ...
            for (let i = 0; i < unknownLayoutInvalidation.length; i++) {
                unknownLayoutInvalidation[i].args = {
                    data: {
                        nodeId: i + 31,
                        reason: _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.TraceEvents.LayoutInvalidationReason.UNKNOWN,
                        nodeName: 'DIV',
                        frame: 'frame-id-123',
                    },
                };
            ...
            for (const e of layoutInvalidationEvents) {
                e.name = _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.TraceEvents.KnownEventName.LayoutInvalidationTracking;
            }
            ...
            model = (0,_testing_TraceHelpers_js__WEBPACK_IMPORTED_MODULE_2__.getBaseTraceParseModelData)();
            modelMut = model;
            ...
            layoutShifts = new _RootCauses_js__WEBPACK_IMPORTED_MODULE_4__.LayoutShiftRootCauses(protocolInterface, { enableIframeRootCauses: true });
        });
        ...
        describe('Injected iframes', () => {
            ...
            it('ignores injected iframes if disabled', async () => {
                layoutShifts = new _RootCauses_js__WEBPACK_IMPORTED_MODULE_4__.LayoutShiftRootCauses(protocolInterface, { enableIframeRootCauses: false });
                const rootCauses = await Promise.all(shifts.map(shift => layoutShifts.rootCausesForEvent(model, shift)));
                assertArrayHasNoNulls(rootCauses);
            ...
            it('ignores events that could not add or resize an iframe', async () => {
                injectedIframeEvents.forEach(e => {
                    ...
                    e.args.data.reason = _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.TraceEvents.LayoutInvalidationReason.SIZE_CHANGED;
                });
                const rootCauses = await Promise.all(shifts.map(shift => layoutShifts.rootCausesForEvent(model, shift)));
                assertArrayHasNoNulls(rootCauses);
                assert(rootCauses.every(cause => cause.iframes.length === 0), 'contained iframe root causes');
        ...
        describe('Font changes', () => {
            ...
            const fontRequests = [
                {
                    dur: _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.Timing.MicroSeconds(2),
                ...
                {
                    dur: _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.Timing.MicroSeconds(30),
                    ts: _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.Timing.MicroSeconds(0),
                    args: {
                        data: {
                            url: fontSource,
            ...
            it('ignores requests for fonts that lie outside the fixed time window from ending at the "font change" layout invalidation event', async () => {
                const optionalFontRequests = [{
                        dur: _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.Timing.MicroSeconds(2),
                        ts: _trace_js__WEBPACK_IMPORTED_MODULE_3__.Types.Timing.MicroSeconds(85),
                        args: {
                            data: {
                                url: fontSource,
                                mimeType: 'font/woff2',
