"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_panels_timeline_EntriesFilter_test_ts"],{

/***/ "./src/panels/timeline/EntriesFilter.test.ts":
/*!***************************************************!*\
  !*** ./src/panels/timeline/EntriesFilter.test.ts ***!
  \***************************************************/
...
/* harmony import */ var _testing_TraceHelpers_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../../testing/TraceHelpers.js */ "./src/testing/TraceHelpers.ts");
...
(0,_testing_EnvironmentHelpers_js__WEBPACK_IMPORTED_MODULE_5__.describeWithEnvironment)('EntriesFilter', function () {
    it('parses a stack and returns an empty list of invisible entries', async function () {
        const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_3__.TraceLoader.traceEngine(this, 'basic-stack.json.gz');
        const stack = new _timeline_js__WEBPACK_IMPORTED_MODULE_1__.EntriesFilter.EntriesFilter(traceData.Renderer.entryToNode);
        assert.deepEqual([], stack?.invisibleEntries());
    });
    it('supports the user merging an entry into its parent', async function () {
        const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_3__.TraceLoader.traceEngine(this, 'basic-stack.json.gz');
        const mainThread = (0,_testing_TraceHelpers_js__WEBPACK_IMPORTED_MODULE_4__.getMainThread)(traceData.Renderer);
        ...
        const entryTwo = findFirstEntry(mainThread.entries, entry => {
            ...
            return _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Types.TraceEvents.isProfileCall(entry) && entry.callFrame.functionName === 'basicTwo' &&
                entry.dur === 827;
        });
        const stack = new _timeline_js__WEBPACK_IMPORTED_MODULE_1__.EntriesFilter.EntriesFilter(traceData.Renderer.entryToNode);
        if (!stack) {
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.MERGE_FUNCTION, entry: entryTwo });
    ...
    it('adds the parent of the merged entry into the expandableEntries array', async function () {
        ...
        const basicStackOne = findFirstEntry(mainThread.entries, entry => {
            return _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Types.TraceEvents.isProfileCall(entry) && entry.callFrame.functionName === 'basicStackOne' &&
                entry.dur === 827;
        });
    ...
    it('adds the collapsed entry into the expandableEntries array', async function () {
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.COLLAPSE_FUNCTION, entry: entryTwo });
    ...
    it('adds the next visible parent of the merged entry into the expandableEntries array if the direct parent is hidden', async function () {
        const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_3__.TraceLoader.traceEngine(this, 'two-functions-recursion.json.gz');
        const mainThread = (0,_testing_TraceHelpers_js__WEBPACK_IMPORTED_MODULE_4__.getMainThread)(traceData.Renderer);
        /** This stack looks roughly like so (with some events omitted):
         * ======== onclick ============
         * =========== foo =============
        ...
        const firstFooCallEntry = findFirstEntry(mainThread.entries, entry => {
            return _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Types.TraceEvents.isProfileCall(entry) && entry.callFrame.functionName === 'foo' &&
                entry.dur === 233;
        });
        const firstFooCallEndTime = _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Helpers.Timing.eventTimingsMicroSeconds(firstFooCallEntry).endTime;
        const fooCalls = mainThread.entries.filter(entry => {
            const isFooCall = _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Types.TraceEvents.isProfileCall(entry) && entry.callFrame.functionName === 'foo';
            if (!isFooCall) {
                return false;
            }
            const { endTime } = _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Helpers.Timing.eventTimingsMicroSeconds(entry);
        ...
        stack.applyFilterAction({
            type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.COLLAPSE_REPEATING_DESCENDANTS,
        ...
        const foo2Calls = mainThread.entries.filter(entry => {
            const isFoo2Call = _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Types.TraceEvents.isProfileCall(entry) && entry.callFrame.functionName === 'foo2';
            if (!isFoo2Call) {
                return false;
            }
            const { endTime } = _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Helpers.Timing.eventTimingsMicroSeconds(entry);
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.MERGE_FUNCTION, entry: foo2Calls[1] });
    ...
    it('supports collapsing an entry', async function () {
        ...
        const fibonacciCalls = mainThread.entries.filter(entry => {
            const isFibCall = _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Types.TraceEvents.isProfileCall(entry) && entry.callFrame.functionName === 'fibonacci';
            if (!isFibCall) {
                return false;
            }
            const { endTime } = _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Helpers.Timing.eventTimingsMicroSeconds(entry);
            const basicTwoCallEndTime = _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Helpers.Timing.eventTimingsMicroSeconds(basicTwoCallEntry).endTime;
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.COLLAPSE_FUNCTION, entry: basicTwoCallEntry });
    ...
    it('supports collapsing all repeating entries among descendants', async function () {
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.COLLAPSE_REPEATING_DESCENDANTS, entry: firstFooCallEntry });
    ...
    it('supports undo all filter actions by applying context menu undo action', async function () {
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.COLLAPSE_REPEATING_DESCENDANTS, entry: fibonacciCalls[0] });
        // We collapsed identical descendants after the first `foo` entry - so it should not be included in the invisible list itself,
        // but all foo() calls below it in the stack should now be invisible.
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.MERGE_FUNCTION, entry: basicStackOne });
        ...
        const basicThree = findFirstEntry(mainThread.entries, entry => {
            return _models_trace_trace_js__WEBPACK_IMPORTED_MODULE_0__.Types.TraceEvents.isProfileCall(entry) && entry.callFrame.functionName === 'basicThree' &&
                entry.dur === 827;
        });
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.UNDO_ALL_ACTIONS, entry: basicTwoCallEntry });
    ...
    it('supports resetting all hidden children of a selected entry', async function () {
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.RESET_CHILDREN, entry: foo2Calls[1] });
    ...
    it('correctly assigns a visible parent to expandable entries if the direct parent is not visible', async function () {
        ...
        stack.applyFilterAction({ type: _ui_legacy_components_perf_ui_perf_ui_js__WEBPACK_IMPORTED_MODULE_2__.FlameChart.FilterAction.MERGE_FUNCTION, entry: anonymousEntryWithInvisibleParent });
