"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_models_trace_handlers_PageLoadMetricsHandler_test_ts"],{

/***/ "./src/models/trace/handlers/PageLoadMetricsHandler.test.ts":
/*!******************************************************************!*\
  !*** ./src/models/trace/handlers/PageLoadMetricsHandler.test.ts ***!
  \******************************************************************/
...
(0,_testing_EnvironmentHelpers_js__WEBPACK_IMPORTED_MODULE_0__.describeWithEnvironment)('PageLoadMetricsHandler', function () {
    describe('contentful paints', () => {
        it('obtains all the FCP and LCP events for all frames', async function () {
            const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.traceEngine(this, 'multiple-navigations-with-iframes.json.gz');
            ...
            const fcpCount = countMetricOcurrences(scoresByMetricName, _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.FCP);
            const lcpCount = countMetricOcurrences(scoresByMetricName, _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.LCP);
        ...
        it('finds the right FCP and LCP events for a trace for a page that was refreshed', async function () {
            const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.traceEngine(this, 'reload-and-trace-page.json.gz');
            const { Meta, PageLoadMetrics } = traceData;
            const { mainFrameId } = Meta;
            const pageLoadMetricsData = PageLoadMetrics.metricScoresByFrameId;
    ...
    describe('markDOMContent frame', () => {
        it('obtains them and assigns them to the correct frames', async function () {
            ...
            // and one for the second.
            assert.strictEqual(pageLoadEventsForMainFrame.size, 2);
            const dclCount = countMetricOcurrences([...pageLoadEventsForMainFrame.values()], _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.DCL);
    ...
    describe('metric scores', () => {
        ...
        it('extracts DOMContentLoaded correctly', () => {
            const domContentLoadedMetrics = getMetricsByName(_trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.DCL);
            assert.strictEqual(domContentLoadedMetrics[0].timing, 34520);
            assert.strictEqual(domContentLoadedMetrics[0].classification, _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.ScoreClassification.UNCLASSIFIED);
            ...
            assert.strictEqual(domContentLoadedMetrics[1].classification, _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.ScoreClassification.UNCLASSIFIED);
            assertMetricNavigationId(domContentLoadedMetrics[1], secondNavigationId);
        });
        it('extracts First Contentful Paint correctly', () => {
            const firstContentfulPaints = getMetricsByName(_trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.FCP);
            assert.strictEqual(firstContentfulPaints[0].timing, 37269);
            assert.strictEqual(firstContentfulPaints[0].classification, _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.ScoreClassification.GOOD);
            ...
            assert.strictEqual(firstContentfulPaints[1].classification, _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.ScoreClassification.GOOD);
            assertMetricNavigationId(firstContentfulPaints[1], secondNavigationId);
        });
        it('extracts Largest Contentful Paint correctly', () => {
            const firstContentfulPaints = getMetricsByName(_trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.LCP);
        ...
        it('extracts First Paint correctly', () => {
            const firstContentfulPaints = getMetricsByName(_trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.FP);
            assert.strictEqual(firstContentfulPaints[0].timing, 37269);
            assert.strictEqual(firstContentfulPaints[0].classification, _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.ScoreClassification.UNCLASSIFIED);
            assertMetricNavigationId(firstContentfulPaints[0], firstNavigationId);
            assert.strictEqual(firstContentfulPaints[1].timing, 42389);
            assert.strictEqual(firstContentfulPaints[1].classification, _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.ScoreClassification.UNCLASSIFIED);
            assertMetricNavigationId(firstContentfulPaints[1], secondNavigationId);
        });
        it('extracts Load correctly', () => {
            const firstContentfulPaints = getMetricsByName(_trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.MetricName.L);
    ...
    describe('FLEDGE fenced frames', () => {
        it('is able to parse a trace containing fenced frames without erroring', async function () {
            const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.traceEngine(this, 'fenced-frame-fledge.json.gz');
    ...
    describe('Marker events', () => {
        ...
        it('extracts all marker events from a trace correctly', () => {
            for (const metricName of _trace_js__WEBPACK_IMPORTED_MODULE_2__.Types.TraceEvents.MarkerName) {
                ...
                assert.isTrue(markerEventsOfThisType.every(marker => _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.PageLoadMetrics.getFrameIdForPageLoadEvent(marker) === mainFrameId));
            }
        });
        ...
        it('only stores the largest contentful paint with the highest candidate index', async function () {
            const { traceData } = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.traceEngine(this, 'multiple-lcp-main-frame.json.gz');
            const { PageLoadMetrics } = traceData;
            const pageLoadMarkers = PageLoadMetrics.allMarkerEvents;
            const largestContentfulPaints = pageLoadMarkers.filter(_trace_js__WEBPACK_IMPORTED_MODULE_2__.Types.TraceEvents.isTraceEventLargestContentfulPaintCandidate);
