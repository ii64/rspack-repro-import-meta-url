"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_models_bindings_PresentationConsoleMessageHelper_test_ts"],{

/***/ "./src/models/bindings/PresentationConsoleMessageHelper.test.ts":
/*!**********************************************************************!*\
  !*** ./src/models/bindings/PresentationConsoleMessageHelper.test.ts ***!
  \**********************************************************************/
...
/* harmony import */ var _testing_MockExecutionContext_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../../testing/MockExecutionContext.js */ "./src/testing/MockExecutionContext.ts");
/* harmony import */ var _workspace_workspace_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ../workspace/workspace.js */ "./src/models/workspace/workspace.ts");
/* harmony import */ var _bindings_js__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./bindings.js */ "./src/models/bindings/bindings.ts");
...
async function addMessage(helper, target, url) {
    ...
    const message = new _core_sdk_sdk_js__WEBPACK_IMPORTED_MODULE_1__.ConsoleModel.ConsoleMessage(target.model(_core_sdk_sdk_js__WEBPACK_IMPORTED_MODULE_1__.RuntimeModel.RuntimeModel), _core_common_common_js__WEBPACK_IMPORTED_MODULE_0__.Console.FrontendMessageSource.ConsoleAPI, _generated_protocol_js__WEBPACK_IMPORTED_MODULE_2__.Log.LogEntryLevel.Error, 'test message', details);
    const level = _workspace_workspace_js__WEBPACK_IMPORTED_MODULE_7__.UISourceCode.Message.Level.Error;
    await helper.addMessage(new _workspace_workspace_js__WEBPACK_IMPORTED_MODULE_7__.UISourceCode.Message(level, message.messageText), message);
    return message;
}
async function addUISourceCode(helper, url) {
    const uiSourceCodeAddedSpy = sinon.stub(helper, 'uiSourceCodeAddedForTest');
    const uiSourceCodeAddedDonePromise = (0,_testing_ExpectStubCall_js__WEBPACK_IMPORTED_MODULE_4__.expectCall)(uiSourceCodeAddedSpy);
    const workspace = _workspace_workspace_js__WEBPACK_IMPORTED_MODULE_7__.Workspace.WorkspaceImpl.instance();
    const project = new _bindings_js__WEBPACK_IMPORTED_MODULE_8__.ContentProviderBasedProject.ContentProviderBasedProject(workspace, 'test-project', _workspace_workspace_js__WEBPACK_IMPORTED_MODULE_7__.Workspace.projectTypes.Network, 'test-project', false);
    const uiSourceCode = new _workspace_workspace_js__WEBPACK_IMPORTED_MODULE_7__.UISourceCode.UISourceCode(project, url, _core_common_common_js__WEBPACK_IMPORTED_MODULE_0__.ResourceType.ResourceType.fromMimeType('application/text'));
...
async function addScript(helper, debuggerModel, executionContext, url) {
    ...
    const parsedScriptSourceDonePromise = (0,_testing_ExpectStubCall_js__WEBPACK_IMPORTED_MODULE_4__.expectCall)(scriptParsedSpy);
    ...
    await _bindings_js__WEBPACK_IMPORTED_MODULE_8__.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().pendingLiveLocationChangesPromise();
    const uiSourceCode = _bindings_js__WEBPACK_IMPORTED_MODULE_8__.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().uiSourceCodeForScript(script);
    assert.exists(uiSourceCode);
    return uiSourceCode;
}
async function addStyleSheet(helper, cssModel, url) {
    const styleSheetAddedSpy = sinon.stub(helper, 'styleSheetAddedForTest');
    const styleSheetAddedDonePromise = (0,_testing_ExpectStubCall_js__WEBPACK_IMPORTED_MODULE_4__.expectCall)(styleSheetAddedSpy);
    const header = {
        styleSheetId: 'styleSheet',
        frameId: 'frameId',
        sourceURL: url,
        origin: _generated_protocol_js__WEBPACK_IMPORTED_MODULE_2__.CSS.StyleSheetOrigin.Regular,
        title: '',
        disabled: false,
        isInline: false,
        isMutable: false,
    ...
    await _bindings_js__WEBPACK_IMPORTED_MODULE_8__.CSSWorkspaceBinding.CSSWorkspaceBinding.instance().pendingLiveLocationChangesPromise();
    const uiSourceCode = _workspace_workspace_js__WEBPACK_IMPORTED_MODULE_7__.Workspace.WorkspaceImpl.instance().uiSourceCodeForURL(url);
    assert.exists(uiSourceCode);
    return uiSourceCode;
}
(0,_testing_MockConnection_js__WEBPACK_IMPORTED_MODULE_5__.describeWithMockConnection)('PresentationConsoleMessageHelper', () => {
    const url = 'http://example.test/test.css';
    let helper;
    let executionContext;
    let cssModel;
    beforeEach(() => {
        executionContext = new _testing_MockExecutionContext_js__WEBPACK_IMPORTED_MODULE_6__.MockExecutionContext((0,_testing_EnvironmentHelpers_js__WEBPACK_IMPORTED_MODULE_3__.createTarget)());
        const { debuggerModel } = executionContext;
        assert.exists(debuggerModel);
        helper = new _bindings_js__WEBPACK_IMPORTED_MODULE_8__.PresentationConsoleMessageHelper.PresentationSourceFrameMessageHelper();
        ...
        const resourceMapping = new _bindings_js__WEBPACK_IMPORTED_MODULE_8__.ResourceMapping.ResourceMapping(targetManager, workspace);
        _bindings_js__WEBPACK_IMPORTED_MODULE_8__.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance({ forceNew: true, resourceMapping, targetManager });
        _bindings_js__WEBPACK_IMPORTED_MODULE_8__.CSSWorkspaceBinding.CSSWorkspaceBinding.instance({ forceNew: true, resourceMapping, targetManager });
    });
