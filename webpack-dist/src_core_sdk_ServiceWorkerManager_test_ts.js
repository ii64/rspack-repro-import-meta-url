"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_core_sdk_ServiceWorkerManager_test_ts"],{

/***/ "./src/core/sdk/ServiceWorkerManager.test.ts":
/*!***************************************************!*\
  !*** ./src/core/sdk/ServiceWorkerManager.test.ts ***!
  \***************************************************/
...
describe('ServiceWorkerVersion', () => {
    ...
    it('identifies when the worker is in installing mode', () => {
        let version = makeVersion(REGISTRATION_PAYLOAD, { ...VERSION_PAYLOAD, status: 'new' });
        assert.strictEqual(version.mode(), _sdk_js__WEBPACK_IMPORTED_MODULE_0__.ServiceWorkerManager.ServiceWorkerVersion.Modes.Installing);
    ...
    it('identifies when the worker is in waiting mode', () => {
        ...
        assert.strictEqual(version.mode(), _sdk_js__WEBPACK_IMPORTED_MODULE_0__.ServiceWorkerManager.ServiceWorkerVersion.Modes.Waiting);
    });
    it('identifies when the worker is in active mode', () => {
        let version = makeVersion(REGISTRATION_PAYLOAD, { ...VERSION_PAYLOAD, status: 'activating' });
        assert.strictEqual(version.mode(), _sdk_js__WEBPACK_IMPORTED_MODULE_0__.ServiceWorkerManager.ServiceWorkerVersion.Modes.Active);
        version = makeVersion(REGISTRATION_PAYLOAD, { ...VERSION_PAYLOAD, status: 'activated' });
        assert.strictEqual(version.mode(), _sdk_js__WEBPACK_IMPORTED_MODULE_0__.ServiceWorkerManager.ServiceWorkerVersion.Modes.Active);
    });
    it('identifies when the worker is in redundant mode', () => {
        ...
        assert.strictEqual(version.mode(), _sdk_js__WEBPACK_IMPORTED_MODULE_0__.ServiceWorkerManager.ServiceWorkerVersion.Modes.Redundant);
    });
