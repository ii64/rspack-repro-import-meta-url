"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_core_sdk_SourceMapScopes_test_ts"],{

/***/ "./src/core/sdk/SourceMapScopes.test.ts":
/*!**********************************************!*\
  !*** ./src/core/sdk/SourceMapScopes.test.ts ***!
  \**********************************************/
...
/* harmony import */ var _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../../testing/SourceMapEncoder.js */ "./src/testing/SourceMapEncoder.ts");
...
const { decodeOriginalScopes, decodeGeneratedRanges } = _sdk_js__WEBPACK_IMPORTED_MODULE_1__.SourceMapScopes;
describe('decodeOriginalScopes', () => {
    it('throws for empty input', () => {
        assert.throws(() => decodeOriginalScopes([''], []));
    });
    it('throws for unexpected commas', () => {
        const brokenScopes = (0,_testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.encodeVlq)(42) + ',' + (0,_testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.encodeVlq)(21);
        assert.throws(() => decodeOriginalScopes([brokenScopes], []), /Unexpected char ','/);
    });
    it('throws for missing "end" item', () => {
        const names = [];
        const brokenScopes = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global').build();
    ...
    it('throws if positions of subsequent start/end items are not monotonically increasing', () => {
        const names = [];
        const scopes = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 40, 'global').start(0, 25, 'function').end(0, 30).end(0, 50).build();
        assert.throws(() => decodeOriginalScopes([scopes], names), /Malformed/);
    });
    it('decodes a global scope', () => {
        const names = [];
        const scope = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global').end(5, 0).build();
    ...
    it('ignores all but the first global scope (multiple top-level siblings)', () => {
        const names = [];
        const scope = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global').end(5, 0).start(10, 0, 'global').end(20, 0).build();
        const originalScopes = decodeOriginalScopes([scope], names);
        assert.lengthOf(originalScopes, 1);
        const { root, scopeForItemIndex } = originalScopes[0];
        assert.deepEqual(root.start, { line: 0, column: 0 });
    ...
    it('decodes nested scopes', () => {
        const names = [];
        const scope = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names)
            .start(0, 0, 'global')
            .start(2, 5, 'function')
            .start(4, 10, 'block')
            .end(6, 5)
...
describe('decodeGeneratedRanges', () => {
    ...
    it('throws for missing "end" item', () => {
        const brokenRanges = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([]).start(0, 0).build();
        assert.throws(() => decodeGeneratedRanges(brokenRanges, [], []), /Malformed/);
    });
    it('decodes a single range', () => {
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([]).start(0, 0).end(5, 0).build();
    ...
    it('decodes multiple top-level ranges', () => {
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([]).start(0, 0).end(0, 10).start(0, 11).end(0, 20).build();
    ...
    it('decodes a nested range on a single line', () => {
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([]).start(0, 0).start(0, 5).end(0, 10).end(0, 15).build();
        const [generatedRange] = decodeGeneratedRanges(range, [], []);
        assert.deepEqual(generatedRange.start, { line: 0, column: 0 });
    ...
    it('decodes sibling ranges on a single line', () => {
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([]).start(0, 0).start(0, 5).end(0, 10).start(0, 15).end(0, 20).end(0, 25).build();
        const [generatedRange] = decodeGeneratedRanges(range, [], []);
        assert.deepEqual(generatedRange.start, { line: 0, column: 0 });
    ...
    it('decodes nested and sibling ranges over multiple lines', () => {
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([])
            .start(0, 0)
            .start(5, 0)
            .start(10, 8)
            .end(15, 4)
    ...
    it('throws if the definition references has an invalid source index', () => {
        const names = [];
        const originEncodedScpoes = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(2, 0, 'function').end(5, 0).build();
        const originalScopes = decodeOriginalScopes([originEncodedScpoes], names);
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([]).start(0, 0, { definition: { sourceIdx: 1, scopeIdx: 0 } }).end(0, 20).build();
    ...
    it('throws if the definition references has an invalid scope index', () => {
        ...
        const originEncodedScpoes = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(2, 0, 'function').end(5, 0).build();
        const originalScopes = decodeOriginalScopes([originEncodedScpoes], names);
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([]).start(0, 0, { definition: { sourceIdx: 0, scopeIdx: 4 } }).end(0, 20).build();
    ...
    it('decodes original scope (definition) references', () => {
        const names = [];
        const originEncodedScpoes = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global').start(5, 0, 'function').end(10, 0).end(20, 0).build();
        const originalScopes = decodeOriginalScopes([originEncodedScpoes], names);
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([])
    ...
    it('decodes original scope (definition) references across multiple original sources', () => {
        const names = [];
        const originEncodedScopes1 = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global').start(5, 0, 'function').end(10, 0).end(20, 0).build();
        const originEncodedScopes2 = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global').start(5, 0, 'function').end(10, 0).end(20, 0).build();
    ...
    it('throws if an inlined range\'s callsite references an invalid source index', () => {
        ...
        const originEncodedScpoes = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(2, 0, 'function').end(5, 0).build();
        const originalScopes = decodeOriginalScopes([originEncodedScpoes], names);
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([]).start(0, 0, { callsite: { sourceIdx: 1, line: 0, column: 0 } }).end(0, 20).build();
        assert.throws(() => decodeGeneratedRanges(range, originalScopes, []), /Invalid source index/);
    });
    it('decodes multiple callsite references in the same source file and the same line', () => {
        const names = [];
        const originEncodedScpoes = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global').start(1, 0, 'function').end(4, 0).end(10, 0).build();
        const originalScopes = decodeOriginalScopes([originEncodedScpoes], names);
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder([])
            .start(0, 0, { definition: { sourceIdx: 0, scopeIdx: 0 } })
    ...
    it('decodes multiple callsite refrences over multiple source files', () => {
        ...
        const names = [];
        const originalEncodedScopes1 = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global').start(1, 0, 'function').end(4, 0).end(10, 0).build();
        const originalEncodedScopes2 = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global').end(10, 0).build();
    ...
    it('decodes bindings that are available/unavailable for the full range', () => {
        const names = [];
        const originalEncodedScopes = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global', undefined, ['foo', 'bar', 'baz']).end(10, 0).build();
        const originalScopes = decodeOriginalScopes([originalEncodedScopes], names);
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder(names)
    ...
    it('decodes bindings that are available with different expressions throughout a range', () => {
        const names = [];
        const originalEncodedScopes = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.OriginalScopeBuilder(names).start(0, 0, 'global', undefined, ['foo']).end(10, 0).build();
        const originalScopes = decodeOriginalScopes([originalEncodedScopes], names);
        const range = new _testing_SourceMapEncoder_js__WEBPACK_IMPORTED_MODULE_0__.GeneratedRangeBuilder(names)
            .start(0, 0, {
