"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_models_trace_handlers_FramesHandler_test_ts"],{

/***/ "./src/models/trace/handlers/FramesHandler.test.ts":
/*!*********************************************************!*\
  !*** ./src/models/trace/handlers/FramesHandler.test.ts ***!
  \*********************************************************/
...
async function processTrace(events) {
    ...
    for (const handlerName of handlersInOrder) {
        const handler = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers[handlerName];
        handler.reset();
    ...
    for (const event of events) {
        for (const handlerName of handlersInOrder) {
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers[handlerName].handleEvent(event);
        }
    }
    for (const handlerName of handlersInOrder) {
        const handler = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers[handlerName];
...
(0,_testing_MockConnection_js__WEBPACK_IMPORTED_MODULE_0__.describeWithMockConnection)('FramesHandler', () => {
    it('can parse out a trace and return the frames', async function () {
        const rawEvents = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'web-dev-with-commit.json.gz');
        await processTrace(rawEvents);
        const parsedFrames = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Frames.data().frames;
    ...
    it('can create LayerPaintEvents from Paint and snapshot events', async function () {
        ...
        // to process it.
        this.timeout(20000);
        const rawEvents = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'web-dev-with-advanced-instrumentation.json.gz');
        await processTrace(rawEvents);
        const parsedFrames = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Frames.data().frames;
    ...
    it('can return frames within a given window', async function () {
        ...
        const parsedFrames = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Frames.data().frames;
        assert.lengthOf(parsedFrames, 18);
        const startTime = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Types.Timing.MicroSeconds(parsedFrames[0].startTime);
        const endTime = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Types.Timing.MicroSeconds(parsedFrames[3].endTime);
        const framesWithinWindow = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Frames.framesWithinWindow(parsedFrames, startTime, endTime);
...
describe('FramesHandler', () => {
    it('visualizes zero frames when no BeginFrames are added', () => {
        const beginFrameQueue = new _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Frames.TimelineFrameBeginFrameQueue();
