"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_core_sdk_Cookie_test_ts"],{

/***/ "./src/core/sdk/Cookie.test.ts":
/*!*************************************!*\
  !*** ./src/core/sdk/Cookie.test.ts ***!
  \*************************************/
...
describe('Cookie', () => {
    it('can be instantiated without issues', () => {
        const cookie = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value');
    ...
    it('can be created from a protocol Cookie with all optional fields set', () => {
        ...
        const cookie = _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.fromProtocolCookie({
            domain: '.example.com',
            expires: expires / 1000,
            httpOnly: true,
            name: 'name',
            path: '/test',
            sameSite: _generated_protocol_js__WEBPACK_IMPORTED_MODULE_0__.Network.CookieSameSite.Strict,
            sameParty: false,
            secure: true,
            session: false,
            size: 23,
            value: 'value',
            priority: _generated_protocol_js__WEBPACK_IMPORTED_MODULE_0__.Network.CookiePriority.High,
            sourcePort: 443,
            sourceScheme: _generated_protocol_js__WEBPACK_IMPORTED_MODULE_0__.Network.CookieSourceScheme.Secure,
        ...
        assert.strictEqual(cookie.sourceScheme(), _generated_protocol_js__WEBPACK_IMPORTED_MODULE_0__.Network.CookieSourceScheme.Secure);
    ...
    it('can be created from a protocol Cookie with no optional fields set', () => {
        ...
        assert.strictEqual(cookie.sourceScheme(), _generated_protocol_js__WEBPACK_IMPORTED_MODULE_0__.Network.CookieSourceScheme.NonSecure);
    });
    ...
    it('can handle secure urls', () => {
        const cookie = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value');
        cookie.addAttribute(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Attribute.Secure);
        cookie.addAttribute(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Attribute.Domain, 'example.com');
        cookie.addAttribute(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Attribute.Path, '/test');
        assert.strictEqual(String(cookie.url()), 'https://example.com/test');
    });
    ...
    it('can set SDK.Cookie.Attribute used as flags', () => {
        const cookie = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value');
        cookie.addAttribute(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Attribute.HttpOnly);
        assert.strictEqual(cookie.httpOnly(), true);
    });
    ...
    it('can set initialize with a different priority', () => {
        const cookie = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value', null, _generated_protocol_js__WEBPACK_IMPORTED_MODULE_0__.Network.CookiePriority.High);
        assert.strictEqual(cookie.priority(), 'High');
    });
    it('can change the priority', () => {
        const cookie = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value');
        cookie.addAttribute(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Attribute.Priority, 'Low');
    ...
    it('can calculate the expiration date for max age cookies', () => {
        ...
        cookie.addAttribute(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Attribute.MaxAge, '3600');
    ...
    it('can calculate the expiration date for cookies with expires attribute', () => {
        ...
        const now = new Date();
        const expires = Math.floor(now.getTime()) + 3600 * 1000;
        cookie.addAttribute(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Attribute.Expires, expires);
        const expiresDate = cookie.expiresDate(now);
        assert.strictEqual(expiresDate.toISOString(), new Date(expires).toISOString());
    });
    it('can check if a cookie domain matches a given host', () => {
        assert.isTrue(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.isDomainMatch('example.com', 'example.com'));
        assert.isFalse(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.isDomainMatch('www.example.com', 'example.com'));
        assert.isTrue(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.isDomainMatch('.example.com', 'example.com'));
        assert.isTrue(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.isDomainMatch('.example.com', 'www.example.com'));
        assert.isFalse(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.isDomainMatch('.www.example.com', 'example.com'));
        assert.isFalse(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.isDomainMatch('example.com', 'example.de'));
        assert.isFalse(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.isDomainMatch('.example.com', 'example.de'));
        assert.isFalse(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.isDomainMatch('.example.de', 'example.de.vu'));
        assert.isFalse(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie.isDomainMatch('example.com', 'notexample.com'));
    });
    it('detects the Partitioned attribute in the Set-Cookie header', () => {
        const cookie = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value');
        cookie.addAttribute(_sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Attribute.Partitioned);
    ...
    it('can compare partition keys', () => {
        const unpartitionedCookie = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value');
        assert.isFalse(unpartitionedCookie.partitioned());
        assert.isFalse(Boolean(unpartitionedCookie.partitionKey()));
        const partitionedCookie = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value');
        ...
        const differentHasCrossSiteAncestor = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value');
        ...
        const differentTopLevel = new _sdk_js__WEBPACK_IMPORTED_MODULE_1__.Cookie.Cookie('name', 'value');
