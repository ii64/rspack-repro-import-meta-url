"use strict";
(self["webpackChunkrspack_repro"] = self["webpackChunkrspack_repro"] || []).push([["src_models_trace_handlers_MetaHandler_test_ts"],{

/***/ "./src/models/trace/handlers/MetaHandler.test.ts":
/*!*******************************************************!*\
  !*** ./src/models/trace/handlers/MetaHandler.test.ts ***!
  \*******************************************************/
...
describe('MetaHandler', function () {
    ...
    beforeEach(async function () {
        ...
        try {
            defaultTraceEvents = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'basic.json.gz');
        }
        catch (error) {
            assert.fail(error);
            return;
    ...
    describe('browser process ID', function () {
        it('obtains the PID if present', async () => {
            ...
            const data = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.data();
    ...
    describe('renderer process ID', function () {
        it('obtains the PID if present', async () => {
            ...
            assert.deepStrictEqual([...data.topLevelRendererIds], [_trace_js__WEBPACK_IMPORTED_MODULE_2__.Types.TraceEvents.ProcessID(8051)]);
        });
    });
    describe('navigations', function () {
        ...
        it('provides a list of main frame only navigations', async function () {
            const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'multiple-navigations-with-iframes.json.gz');
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.reset();
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.initialize();
    ...
    describe('frames', function () {
        ...
        it('finds the main frame ID for a trace that started with a page reload', async function () {
            const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'reload-and-trace-page.json.gz');
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.reset();
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.initialize();
            for (const event of events) {
                _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.handleEvent(event);
    ...
    describe('finding GPU thread and main frame', function () {
        it('finds the GPU process and GPU Thread', async function () {
            const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'threejs-gpu.json.gz');
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.reset();
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.initialize();
            for (const event of events) {
                _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.handleEvent(event);
            ...
            await _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.finalize();
            const { gpuProcessId, gpuThreadId } = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.data();
        ...
        it('handles traces that do not have a GPU thread and returns undefined for the thread ID', async function () {
            const traceEventsWithNoGPUThread = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'forced-layouts-and-no-gpu.json.gz');
    ...
    it('obtains renderer process IDs when there are no navigations', async function () {
        let traceEvents;
        try {
            traceEvents = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'threejs-gpu.json.gz');
        }
        catch (error) {
            assert.fail(error);
            return;
    ...
    it('handles multiple renderers from navigations', async function () {
        let traceEvents;
        try {
            traceEvents = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'multiple-top-level-renderers.json.gz');
        }
        catch (error) {
            assert.fail(error);
            return;
    ...
    it('handles multiple renderers from navigations where a process handled multiple URLs ', async function () {
        let traceEvents;
        try {
            traceEvents = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'simple-js-program.json.gz');
        }
        catch (error) {
            assert.fail(error);
            return;
    ...
    it('calculates trace bounds correctly', async function () {
        let traceEvents;
        try {
            traceEvents = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'basic.json.gz');
        }
        catch (error) {
            assert.fail(error);
            return;
        ...
        const expectedMin = 50442438975;
        const expectedMax = 50442438976;
    ...
    it('calculates the min trace bound correctly if no TracingStartedInBrowser event is found', async function () {
        const baseEvents = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'basic.json.gz');
    ...
    it('ignores ::UMA Events', async function () {
        ...
        try {
            ...
            traceEvents = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'web-dev.json.gz');
        }
        catch (error) {
            assert.fail(error);
            return;
        ...
        const data = _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.data();
        const { max, min, range, } = data.traceBounds;
        const expectedMin = 1020034823047;
        const expectedMax = 1020036087961;
        assert.strictEqual(min, expectedMin, 'Min calculated incorrectly');
        assert.strictEqual(max, expectedMax, 'Max calculated incorrectly');
        assert.strictEqual(range, expectedMax - expectedMin, 'Range calculated incorrectly');
    });
    ...
    it('can handle edge cases where there are multiple navigations with the same ID', async function () {
        ...
        const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'multiple-navigations-same-id.json.gz');
    ...
    it('marks a generic trace as generic', async function () {
        const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'generic-about-tracing.json.gz');
        for (const event of events) {
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.handleEvent(event);
        }
        await _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.finalize();
    ...
    it('marks a web trace as being not generic', async function () {
        const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'web-dev-with-commit.json.gz');
        for (const event of events) {
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.handleEvent(event);
        }
        await _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.finalize();
    ...
    it('will alter the main frame URL based on the first main frame navigation', async function () {
        ...
        const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'web-dev-initial-url.json.gz');
        for (const event of events) {
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.handleEvent(event);
        }
        await _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.finalize();
    ...
    it('returns a list of processes and process_name events', async function () {
        ...
        assert.deepEqual(pidsToNames, [
            [_trace_js__WEBPACK_IMPORTED_MODULE_2__.Types.TraceEvents.ProcessID(37605), 'Browser'],
            [_trace_js__WEBPACK_IMPORTED_MODULE_2__.Types.TraceEvents.ProcessID(48544), 'Renderer'],
            [_trace_js__WEBPACK_IMPORTED_MODULE_2__.Types.TraceEvents.ProcessID(37613), 'GPU Process'],
            [_trace_js__WEBPACK_IMPORTED_MODULE_2__.Types.TraceEvents.ProcessID(48531), 'Renderer'],
        ]);
    });
    it('does not set a frame as a main frame if it has no URL.', async function () {
        ...
        const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'wrong-main-frame-bug.json.gz');
        for (const event of events) {
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.handleEvent(event);
        }
        await _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.finalize();
    ...
    it('will use isOutermostMainFrame to determine the main frame from the TracingStartedInBrowser event if it is present', async function () {
        const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'web-dev-outermost-frames.json.gz');
        for (const event of events) {
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.handleEvent(event);
        }
        await _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.finalize();
    ...
    it('will use isInPrimaryPage along with isOutermostMainFrame to identify the main frame from TracingStartedInBrowser', async function () {
        ...
        const events = await _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_1__.TraceLoader.rawEvents(this, 'primary-page-frame.json.gz');
        for (const event of events) {
            _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.handleEvent(event);
        }
        await _trace_js__WEBPACK_IMPORTED_MODULE_2__.Handlers.ModelHandlers.Meta.finalize();
