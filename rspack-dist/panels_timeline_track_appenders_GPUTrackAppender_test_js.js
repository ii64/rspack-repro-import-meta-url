...
"./panels/timeline/track_appenders/GPUTrackAppender.test.js": (function (__unused_webpack_module, __webpack_exports__, __webpack_require__) {
...
(0,_testing_EnvironmentHelpers_js__WEBPACK_IMPORTED_MODULE_1__.describeWithEnvironment)('GPUTrackAppender', function() {
        ...
        return _ts_generator(this, function(_state) {
            switch(_state.label){
                case 0:
                    return [
                        4,
                        _testing_TraceLoader_js__WEBPACK_IMPORTED_MODULE_2__.TraceLoader.traceEngine(this, 'threejs-gpu.json.gz')
                    ];
                case 1:
    ...
    describe('appendTrackAtLevel', function() {
        ...
        it('creates a flamechart group for the GPU track', function() {
            assert.strictEqual(flameChartData.groups.length, 1);
            assert.strictEqual(flameChartData.groups[0].name, 'GPU');
        });
        it('adds start times correctly', function() {
            var gpuEvents = traceData.GPU.mainGPUThreadTasks;
            var _iteratorNormalCompletion = true, _didIteratorError = false, _iteratorError = undefined;
            try {
                for(var _iterator = gpuEvents[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true){
                    var event = _step.value;
                    var index = entryData.indexOf(event);
                    assert.exists(index);
    ...
    describe('highlightedEntryInfo', function() {
        it('returns the info for a entry correctly', function() {
            var gpuEvents = traceData.GPU.mainGPUThreadTasks;
            var highlightedEntryInfo = gpuTrackAppender.highlightedEntryInfo(gpuEvents[0]);
