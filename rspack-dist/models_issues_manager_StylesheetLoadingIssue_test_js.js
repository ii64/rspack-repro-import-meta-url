...
"./models/issues_manager/StylesheetLoadingIssue.test.js": (function (__unused_webpack_module, __webpack_exports__, __webpack_require__) {
...
(0,_testing_EnvironmentHelpers_js__WEBPACK_IMPORTED_MODULE_1__.describeWithLocale)('StylesheetLoadingIssue', function() {
    ...
    it('can be created for failed stylesheet requests', function() {
        var issueDetails = {
            sourceCodeLocation: {
        ...
        var stylesheetIssues = _issues_manager_issues_manager_js__WEBPACK_IMPORTED_MODULE_4__.StylesheetLoadingIssue.StylesheetLoadingIssue.fromInspectorIssue(mockModel, issue);
        assert.lengthOf(stylesheetIssues, 1);
        var stylesheetIssue = stylesheetIssues[0];
        assert.strictEqual(stylesheetIssue.getCategory(), "Other" /* IssuesManager.Issue.IssueCategory.Other */ );
        assert.deepStrictEqual(stylesheetIssue.sources(), [
            issueDetails.sourceCodeLocation
        ]);
        var _issueDetails_failedRequestInfo = issueDetails.failedRequestInfo, url = _issueDetails_failedRequestInfo.url, requestId = _issueDetails_failedRequestInfo.requestId;
        assert.deepStrictEqual(stylesheetIssue.requests(), [
            {
                url: url,
                requestId: requestId
            }
    ...
    it('can be created for late import rules', function() {
        var issueDetails = {
            sourceCodeLocation: {
                url: 'http://example.com',
                lineNumber: 2,
    ...
    it('correctly aggregates issues', function() {
        var issueDetails = [
            {
                sourceCodeLocation: {
                    url: 'http://example.com',
        ...
        var issues = issueDetails.map(function(details) {
            return _issues_manager_issues_manager_js__WEBPACK_IMPORTED_MODULE_4__.StylesheetLoadingIssue.StylesheetLoadingIssue.fromInspectorIssue(mockModel, createProtocolIssue(details));
        }).flat();
        ...
        assert.deepStrictEqual(Array.from(aggregatedIssues[0].sources()), [
            issueDetails[0].sourceCodeLocation,
            issueDetails[1].sourceCodeLocation
        ]);
        assert.deepStrictEqual(Array.from(aggregatedIssues[1].sources()), [
            issueDetails[2].sourceCodeLocation,
            issueDetails[3].sourceCodeLocation
        ]);
        assert.deepStrictEqual(Array.from(aggregatedIssues[0].requests()), []);
        var _issueDetails__failedRequestInfo = issueDetails[3].failedRequestInfo, url = _issueDetails__failedRequestInfo.url, requestId = _issueDetails__failedRequestInfo.requestId;
        assert.exists(requestId);
        assert.deepStrictEqual(Array.from(aggregatedIssues[1].requests()), [
            {
                url: url,
                requestId: requestId
            }
